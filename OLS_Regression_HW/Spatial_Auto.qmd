...
bandwidth_fixed<-gwr.sel(formula=LNMEDINC~LNMEDHVAL+PCTVACANT, 
            data=regress_spatial,
            method = "aic",
            adapt = FALSE)
```
...
gwr_model<-gwr(formula=LNMEDINC~LNMEDHVAL+PCTVACANT,
              data=regress_spatial,
...
gwr_model_fixed<-gwr(formula=LNMEDINC~LNMEDHVAL+PCTVACANT,
              data=regress_spatial,
...
```{r}
# Create readable break labels 
breaks <- c(-Inf, -6, -4, -2, 0, 2, 4, 6, Inf)
labels <- c("Less than -6", "-6 to -4", "-4 to -2", "-2 to 0", 
            "0 to 2", "2 to 4", "4 to 6", "Greater than 6")

gwr_results<-as.data.frame(gwr_model$SDF)
regress_spatial$coefLNMEDHVALst<-gwr_results$LNMEDHVAL/gwr_results$LNMEDHVAL_se
regress_spatial$coefPCTVACANTst<-gwr_results$PCTVACANT/gwr_results$PCTVACANT_se

regress_spatial$gwrE<-gwr_results$gwr.e
regress_spatial$localR2<-gwr_results$localR2

# LNMEDHVAL standardized coefficient map
coefLNMEDHVAL <- tm_shape(regress_spatial) +
  tm_polygons(
    col = "coefLNMEDHVALst",
    palette = "-RdBu",
    breaks = breaks,
    labels = labels,        # custom labels
    style = "fixed"         # fixed breaks
  ) +
  tm_layout(
    frame = FALSE,
    legend.position = c("right", "bottom"),   # bottom-right corner
    legend.title.size = 0.8,
    legend.text.size = 0.7
  ) +
  tm_title("Median House Value (Log)")

# PCTVACANT standardized coefficient map
coefPCTVACANT <- tm_shape(regress_spatial) +
  tm_polygons(
    col = "coefPCTVACANTst",
    palette = "-RdBu",
    breaks = breaks,
  ...
  tm_title("Percentage of Housing Vacant") +
  tm_layout(
    frame = FALSE,
...
tmap_arrange(
  ...
  PCTVACANT_coef,
  ncol = 2,
  widths = c(1, 1),  # equal map width
  heights = c(1)     # larger overall map height
)

# WHY WONT MY FUCKING LABELS WORK ASDDFJKGHJAKDGLHL
```
...
y <- regress_data$LNMEDINC # Dependent variable vector
