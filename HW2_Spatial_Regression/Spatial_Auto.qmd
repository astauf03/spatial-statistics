...
regress_data$OLS_LAG <- lag.listw(queenlist, regress_data$OLS_RESIDU)
...
summary(lm(moran_RESIDU$x ~ moran_RESIDU$wx))
```
...
#Note, whitestrap has installation constraints on CRAN, below is the code one could run if they wanted to see the differentiation between Breusch-Pagan and White Test.
#white_test(ols_reg)   
```
...
```{r}
arranged_coef <- tmap_arrange(coefLNNBELPOV, coefPCTBACHMOR, coefPCTSINGLES, coefPCTVACANT, nrow = 2, ncol = 2)

arranged_coef

#tmap_save(arranged_coef, filename = "HW2_Plots/arranged_coef_map_rows.png", width = 8, height = 8, dpi = 300)
```
